...
class CBSLogger(BaseLogger):
    ...
    def log_request(self, service: str, operation: str, payload: dict):
        self._setup_logger()
        self.log_info(f"CBS Request - Service: {service}, Operation: {operation}, Payload: {payload}")
        
    def log_response(self, response: dict):
        self._setup_logger()
        self.log_info(f"Parsed CBS response body: {response}")
...
class AuthLogger(BaseLogger):
    ...
    def log_login(self, user_id: str, status: str):
        self._setup_logger()
        self.log_info(f"Login attempt - User: {user_id}, Status: {status}")
        
    def log_logout(self, user_id: str):
        self._setup_logger()
        self.log_info(f"User logged out: {user_id}")
